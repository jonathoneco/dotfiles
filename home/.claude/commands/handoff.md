...
3. **Beans activity**: `bn list --status=in_progress` for claimed issues, `bn list --status=closed --since=today` for completed issues (if `bn` supports `--since`, otherwise skip)
