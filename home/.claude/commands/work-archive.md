...
3. This is belt-and-suspenders with the `review-verify.sh` stop hook (which also blocks)
...
3. Parse the diff for new rows in the "Deprecated Approaches" table:
   ...
   - Normalize to lowercase kebab-case per spec 00 identifier rules (lowercase, strip parentheticals, strip text after `/`, replace spaces with hyphens, collapse consecutive hyphens)
...
5. **Content grep for newly deprecated** (only if `newly_deprecated` is non-empty): For each newly deprecated item, resolve all document paths (skills: `~/.claude/skills/<name>/SKILL.md`, rules: `.claude/rules/<name>.md`, commands: `.claude/commands/<name>.md` then `~/.claude/commands/<name>.md`). Grep each file for the deprecated technology name (case-insensitive). Flag matches as "undeclared stale reference"
