...
2. Check for critical anti-patterns:
   - `_, _ =` — swallowed error returns
   - `_ = .*\.Exec\(` — swallowed DB exec error
   - `_ = .*\.Render\(` — swallowed template render error
   - `if err == nil` without else branch
   - Fabricated data (return synthetic values on nil/error paths)
...
5. **Full review optional**: User can run `/work-review` explicitly for a full multi-agent review, but this is optional for Tier 1. The Stop hook (`review-gate.sh`) also fires at session end as an additional safety net.
