...
| 2 | state.json + plan document (if exists in `docs/feature/<name>/`) + beads issue |
...
If a listed file does not exist, note its absence but continue with what's available.

### Step 4: Check for open findings

If `.review/findings.jsonl` exists, count OPEN findings for this task (last line per finding ID).
