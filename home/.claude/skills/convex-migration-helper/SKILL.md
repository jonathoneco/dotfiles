...
## Field renames — bilateral backfill

Convex's schema validator rejects deploys when any row carries an undeclared
field. Dropping a schema field thus requires that every existing row already
lack that field — so the widen-migrate-narrow backfill for a rename must be
**bilateral**: write the new field AND clear the old one in the same per-row
mutation.

A unilateral backfill (writes the new field only) leaves the old-field values
behind on disk, and the schema-narrow commit will fail to deploy with a
validation error against the still-present legacy field.

```typescript
// In the per-row migration body — write new + clear old in one patch.
const patch: { fileCreatedAt?: number; applicationDate?: undefined } = {}
if (!hasFileCreatedAt) {
  patch.fileCreatedAt = hasLegacy ? row.applicationDate : row._creationTime
}
if (hasLegacy) {
  patch.applicationDate = undefined // explicit undefined removes the column
}
await ctx.db.patch(row._id, patch)
```

Setting a field to `undefined` on a patch tells Convex to remove the physical
column from the row even when the schema no longer models that field (you may
need to cast past the type system, since `Doc<...>` has already dropped the
legacy field by the time the narrow commit lands).

The narrow commit (deploy 2) can only ship after this bilateral migration has
run against the deployed environment. Verify with `npx convex data <table>`
that no rows still carry the legacy field before deploying the narrow schema.

Canonical example in the Wrangle codebase: `migrateRowToFileCreatedAt` in
`convex/migrations.ts` (the `applicationDate` → `fileCreatedAt` rename).
