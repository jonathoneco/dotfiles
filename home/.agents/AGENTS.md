...
- **Gardening**: leave the code cleaner than you found it. Reuse what already exists instead of adding a near-copy. Remove what nothing uses, such as dead branches, stale comments, duplicated logic, abstractions with one caller. Put each piece at the right level: a helper that only one file needs stays in that file. Take out wasted work you notice: repeated reads, independent steps run one after another. Choose the clear version over the short one. The test is lift, not location. A cleanup that is small, checked by the existing tests and typecheck, and safe to explain in one PR line goes in, in the files you touched or the ones right beside them. A small defect you find on the way goes in too when the fix is obvious and covered; name it in the PR body so the reviewer sees it as a fix, not a cleanup. Defer only what is heavy: a design change, a migration, many callers, or a step that must wait on production. When Jon asks whether a finding is gardening, answer per finding: in, and do it; or out, with what makes it heavy and where it goes instead, so a skipped finding is never dropped in silence.
