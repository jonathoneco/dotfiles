...
- When Jon says remember, capture, or save this, use the user-scoped OpenBrain capture tool (`capture_thought`). The SessionEnd hook writes a pending session summary automatically. What gets written lands as pending review: it cannot be searched or bind anyone until Jon confirms it, so do not build on your own capture later.
...
- **Babysit the PR**: work the PR until every required check passes. Read the failure with `gh`, fix it, push, and read again. Report when it's green or when a fix needs a decision that is Jon's. Merge only if he says "and merge."
...
- **Code smell**: Jon senses something is off before he can say why, in code, wiring, a design, or prose. Trace it to the mechanism that produces it and report that mechanism before you edit the flagged spot, naming the smell when it has a name, such as a Fowler smell from code-review's baseline. His hunch ("feels provider-specific") is a lead to test, not a conclusion.
