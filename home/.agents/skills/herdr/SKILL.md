...
A workspace can be bound to a configured remote machine; its tabs and splits then run on that machine and `--cwd` is a path there. Pane, agent, read, and wait commands work identically on remote panes — target them by pane ID and ignore where they run. `herdr pane split ... --host NAME` puts a single pane on a machine from anywhere; plugin and overlay panes always run locally.

`herdr host status` shows a machine's connection. `reconnecting` and `suspended` recover on their own (suspended reprobes every five minutes); `herdr host resume NAME` skips the wait. Treat a remote pane that renders but ignores input as a connection problem, not a pane problem — check `herdr host status` first.

When you are running inside a remote pane, your `herdr` commands are relayed to the user's local server and scoped to your own workspace. Reads, prompts, splits, `pane close`, `tab close`, and in-workspace `pane move` work; anything that crosses the workspace boundary or defers (`worktree create`, `host *`, `agent prompt --wait` and other waits through the relay, `pane move --new-workspace`) is refused with `relay_method_denied` or an explicit error — poll `agent get` instead of waiting. To get a new worktree from inside a remote pane, run `git worktree add` yourself, then `herdr tab create --cwd <checkout>`.
...
- Inside a pane on a remote machine, the CLI is restricted to the calling workspace on that machine: `pane close`, `tab close`, and in-workspace `pane move` work there for cleanup and reorganization, but layout, worktree, host, plugin, workspace-close, new-workspace moves, and server methods answer `relay_method_denied` by design. For a worktree, run `git worktree add <path> -b <branch>` in a shell pane and open it with `herdr tab create --cwd <path>` or `herdr pane split --cwd <path>`. Report anything else outside that surface to the user instead of retrying.
