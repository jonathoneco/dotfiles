...
local config_dir = vim.fn.expand("$XDG_CONFIG_HOME") .. "/nvim"
local state_file = config_dir .. "/colorscheme.json"

-- Ensure directory exists
vim.fn.mkdir(vim.fn.fnamemodify(state_file, ":h"), "p")

-- Blacklist of themes to avoid
local blacklist_file = config_dir .. "colorscheme_blacklist.json"
...
function Themes.ColorMyPencils(color)
    ...
    -- General solution: Force complete theme refresh with delay
...
function Themes.load_last_theme()
    ...
    if f then
        ...
        if ok and data and data.theme then
            local success, err = pcall(Themes.ColorMyPencils, data.theme)
            if not success then
                print("Failed to load theme " .. data.theme .. ": " .. (err or "unknown error"))
            end
        end
    end
end
