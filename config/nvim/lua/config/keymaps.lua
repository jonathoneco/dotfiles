...
map("n", "<leader>cb", function()
    ...
    local line_num = vim.fn.line(".")
    local blame = vim.fn.system(string.format("git blame -L %d,%d --porcelain %s 2>/dev/null", line_num, line_num,
        file_path))
    ...
    local result = string.format("%s:%d\nAuthor: %s\nDate: %s\nCommit: %s", rel_path, line_num, author or "unknown",
        date or "unknown", summary or "unknown")
    vim.fn.setreg("+", result)
